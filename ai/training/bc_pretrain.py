...
looks locally symmetric. The heuristic bot (ai/env/rules_bot.py, "hard" survives
...
Two stages, one command:
  ...
  python -m ai.training.bc_pretrain --config ai/config/phase1.yaml \
...
from ai.env.curve_env import CurveEnv
from ai.env.rules_bot import BOT_DIFFICULTIES, RuleBasedBot
from ai.models.algo import build_algo_kwargs, resolve_algo
from ai.models.policy import build_policy_kwargs
from ai.training.curriculum import CurriculumManager
from ai.training.train import build_stage, load_config, make_configs
...
def generate_dataset(
    ...
    from ai.env.observation import vector_dim
...
def train_bc(
    ...
    print(f"[bc] nächster Schritt:  python -m ai.training.train --config <phase.yaml> --init-from {out_path}")
    return out_path
