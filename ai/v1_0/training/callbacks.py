...
returns on termination/truncation - see ai/core/env/curve_env.py), so none of this needs
...
from ai.core.env.opponents import SpecOpponentFactory
...
class MetricsLoggingCallback(BaseCallback):
    ...
    to CSV + JSON lines, so ai/v1_0/reporting/plots.py doesn't need TensorBoard's event
...
class MilestoneReportCallback(BaseCallback):
    """Fires ai/v1_0/reporting/report.py once per crossed step milestone."""
    ...
    def _on_step(self) -> bool:
        for milestone in MILESTONES:
            ...
            self._done.add(milestone)
            try:
                from ai.v1_0.reporting.report import generate_report
